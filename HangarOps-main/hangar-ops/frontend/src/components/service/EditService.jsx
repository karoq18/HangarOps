...
function EditService() {
 ...
 useEffect(() => {
   const fetchServiceData = async () => {
     try {
       const response = await fetch(`http://localhost:8080/get-maintenance/${id}`);
       if (!response.ok) {
       ...
       setSelectedPilot(data.pilot);
       setSelectedOrganization(data.organization);
       setSelectedMechanics(data.mechanic.split(", "));
       setStartDate(data.startDate.substring(0, 10));
       setCompletionDate(data.completionDate.substring(0, 10));
     } catch (err) {
...
const updatedData = {
   ...
   const handleSubmit = async (e) => {
       ...
       setLoading(true);
       try {
         const response = await fetch(`http://localhost:8080/update-maintenance/${id}`, {
           method: 'PUT',
           headers: {
         ...
         <Container>
             <Row className="justify-content-center">
               <Col lg={8}>
                 <Card className="mt-4">
                   <Card.Body>
                    <h3 className="text-uppercase text-center py-3 bg-light border-bottom shadow-sm">Edytuj aktywną usługę</h3>
